...
const SprintIntensity: React.FC<Props> = ({ onInteraction }) => {
  ...
  useEffect(() => {
    ...
    const draw = (t: number) => {
      ...
      // --- animated runner (bottom-right of canvas) ---
      const rx = W - 70;
      const ry = H - 26;
      const ph = phaseRef.current;
      const stride = 7 + e * 9;
      ctx.strokeStyle = anaerobic ? COL.burn : '#cfe9d8';
      ctx.lineWidth = 3;
      ctx.lineCap = 'round';
      const bob = Math.abs(Math.sin(ph)) * 3;
      const hipX = rx;
      const hipY = ry - 18 - bob;
      // head + torso
      ctx.fillStyle = anaerobic ? COL.burn : '#cfe9d8';
      ctx.beginPath();
      ctx.arc(hipX + 2, hipY - 20, 5, 0, Math.PI * 2);
      ctx.fill();
      ctx.beginPath();
      ctx.moveTo(hipX, hipY);
      ctx.lineTo(hipX + 2, hipY - 16);
      ctx.stroke();
      // legs
      ctx.beginPath();
      ctx.moveTo(hipX, hipY);
      ctx.lineTo(hipX + Math.cos(ph) * stride, ry);
      ctx.moveTo(hipX, hipY);
      ctx.lineTo(hipX + Math.cos(ph + Math.PI) * stride, ry);
      ctx.stroke();
      // arms
      ctx.beginPath();
      const shoulderY = hipY - 12;
      ctx.moveTo(hipX + 1, shoulderY);
      ctx.lineTo(hipX + 1 + Math.cos(ph + Math.PI) * (stride * 0.7), shoulderY + 8);
      ctx.moveTo(hipX + 1, shoulderY);
      ctx.lineTo(hipX + 1 + Math.cos(ph) * (stride * 0.7), shoulderY + 8);
      ctx.stroke();
      // gasping marks when anaerobic
      if (anaerobic) {
        ctx.fillStyle = 'rgba(255,255,255,0.6)';
        ctx.font = '11px Arial';
        ctx.fillText('huff', hipX + 8, hipY - 26 - (ph % 1) * 6);
      }
    ...
    <div className="w-full h-full flex flex-col" style={{ color: '#fff' }}>
      <div style={{ flex: 1, minHeight: 250, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
        <canvas
          ...
          style={{ width: '100%', maxWidth: 660, height: 'auto', borderRadius: 14, background: COL.bg, boxShadow: '0 6px 24px rgba(0,0,0,0.35)' }}
        />
      </div>
