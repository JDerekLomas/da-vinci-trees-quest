...
import { SeaUrchinExplorerInteraction } from './interface';
import { useTranslations } from '../../../hooks/useTranslations';
import ToggleSwitch from './toggle-switch';
import ExplorerGraph from './explorer-graph';

const UrchinDensityExplorer: React.FC<{
  interaction: SeaUrchinExplorerInteraction;
}> = ({ interaction }) => {
  const { t } = useTranslations();
  const [showKelp, setShowKelp] = useState(true);
  ...
  const { earlyTrend, lateTrend } = useMemo(() => {
    const early = interaction.urchinData.filter((d) => d.year < 2014);
    const late = interaction.urchinData.filter((d) => d.year >= 2014);
    return {
  ...
  const enrichedData = useMemo(
    () =>
      interaction.urchinData.map((point) => ({
        ...point,
        urchins: Number(point.urchins.toFixed(2)),
        kelp: Number(point.kelp.toFixed(2)),
        earlyTrend:
          point.year < 2014
            ? Math.max(0, Number((earlyTrend.slope * (point.year - 2000) + earlyTrend.intercept).toFixed(2)))
            : null,
        lateTrend:
          point.year >= 2014
            ? Math.max(0, Number((lateTrend.slope * (point.year - 2000) + lateTrend.intercept).toFixed(2)))
            : null,
      })),
    [interaction.urchinData, earlyTrend, lateTrend],
  );
  ...
  const projectedData = useMemo(() => {
    ...
    return Array.from({ length: 11 }, (_, i) => {
      ...
      return {
        ...
        kelp: null,
        earlyTrend: null,
        lateTrend: Math.max(0, Number(projection.toFixed(2))),
      };
    });
  ...
  const dataMax = Math.max(
    ...filteredData
      .map((item) => [item.urchins, item.earlyTrend, item.lateTrend].filter((val) => val !== null))
      .flat(),
  );
    ...
    <div className="w-full flex flex-col items-center">
      ...
      <div className="h-[480px] w-full" role="region" aria-label={t(interaction.ariaLabel || '')} tabIndex={0}>
        <ExplorerGraph
          ...
          xTicks={xTicks}
          yLeftDomain={[0, Math.ceil(dataMax)]}
          yRightDomain={[0, 6]}
          showKelp={showKelp}
...
export default UrchinDensityExplorer;
