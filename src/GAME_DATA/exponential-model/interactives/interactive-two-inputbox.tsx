...
const InteractiveInputBox: React.FC<InteractiveInputBoxProps> = ({
  ...
  useEffect(() => {
    if (isSubmitTriggered || interactionState?.isCorrect) {
      ...
      if (interactionState?.isCorrect && interactionState.value1 && interactionState.value2) {
  ...
  return (
    <div className="relative mt-5 flex flex-col gap-4">
      ...
      <div className="relative w-full flex gap-x-3 items-center text-1.5xl">
        ...
        <div className="relative w-full">
          <input
            ref={input1Ref}
            type="number"
            className={`interactive-input-box w-full mb-2 border rounded-[0.125rem] text-1.5xl font-medium px-4 py-[6px] ${showCheckmark
