...
// ElevenLabs voice mapping for matrix-multiplication quest
...
const extractDialogs = (localeData, gameId, lang) => {
    const dialogs = [];
    const scenes = localeData.scenes || {};
    let sceneIndex = 0;
    for (const sceneKey of Object.keys(scenes)) {
        if (sceneKey === 'common' || sceneKey === 'glossary')
            continue;
...
const generateAudio = async (elevenlabs, text, voiceId, outputPath) => {
    try {
