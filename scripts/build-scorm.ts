...
const GAME_CONFIGS: Record<GameId, GameConfig> = scormConfig;
const GET_DIST_DIR = (gameId: GameId) => path.join(DIST_DIR, gameId);
const GET_SCORM_DIR = (gameId: GameId) => path.join(GET_DIST_DIR(gameId), SCORM_BUILD_DIR);

function createManifest(gameId: GameId): string {
...
function updateManifest(manifestPath: string, gameId: GameId) {
...
function createScormPackage(gameId: GameId) {
  //   // Build the app for the specific game
  //   console.log(`Building ${gameId}...`);
  //   execSync(`tsc -b && VITE_GAME_ID=${gameId} vite build --base='./'`, { stdio: 'inherit' });

  if (!fs.existsSync(TEMP_SCORM_DIR)) {
    fs.mkdirSync(TEMP_SCORM_DIR);
  }

  // copy dist to temp scorm directory
  ...
  // Create scorm build directory if it doesn't exist
  if (!fs.existsSync(GET_SCORM_DIR(gameId))) {
    fs.mkdirSync(GET_SCORM_DIR(gameId));
  }

  // copy dist to scorm build directory
  ...
  if (!fs.existsSync(path.join(GET_DIST_DIR(gameId), SCORM_DIR))) {
    ...
    // delete whatever is inside the scorm directory
  ...
  // Add all files from scorm build directory for the game
  archive.directory(GET_SCORM_DIR(gameId), false);

  archive.finalize();
}

function validateGameId(gameId: string): gameId is GameId {
  return GAMES.includes(gameId as GameId);
}

// Build all games if no specific game is provided
