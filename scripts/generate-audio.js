...
import { voices, inputPath, outputPath, blockElements, VITE_GAME_ID, NARAKEET_API_KEY, VOICE_AI_API_KEY, } from './audio-constants.js';
...
const downloadAudio = async (downloadUrl, outputPath) => {
    try {
        ...
        console.error('[ERROR] : Failed to download:', error.message);
        fs.unlink(outputPath, () => console.log(`[INFO] : Cleaned up file at ${outputPath}`));
        throw error;
    }
};
const processTextAudio = async (text, outputPath, voice, service) => {
    try {
        const response = await service.requestAudioTask(text, voice);
...
const processDialogs = async (lang) => {
    try {
        ...
        const tasks = Object.keys(data.scenes).map(async (sceneKey) => {
            ...
            for (const dialogKey in scene) {
                ...
                const character = dialogKey.split('_')[3];
                if (!voices[character]) {
                ...
                try {
                    ...
                    if (lang === 'en')
                        await processTextAudio(text, outputFilePath, voice, voiceAiAudio);
                    else if (lang === 'es')
                        await processTextAudio(text, outputFilePath, voice, narakeetAudio);
                    else
